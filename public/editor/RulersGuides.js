...
var RulersGuides = function (evt, dragdrop) {
        ...
        Menu = function () {
            var menuList = null,
                ...
                menuItemsList  = [{
                    ...
                    'text': '상세정보 표시', //'상세 정보 표시'
                    'hotkey': 'Ctrl + Alt + I',
                    'alias': 'details'
                }, {
                    'text': 'DOM에 스냅', //'DOM에 스냅'
                    'hotkey': 'Ctrl + Alt + E',
                    'alias': 'snapdom'
                }],
                i = 0;
            ...
            this.render = function () {
                ...
                evt.attach('mousedown', menuBtn, function () {
                    ...
                    toggles.lock.txt.nodeValue = (locked === 0) ? '자 잠금' : '자 잠금 해제';
                    toggles.details.txt.nodeValue = (detailsStatus === 0) ? '상세정보' : '상세정보 숨김';
                    toggles.snapdom.txt.nodeValue = (snapDom === 0) ? 'Dom에 스냅' : 'Dom에 스냅 해제';
    ...
    evt.attach('mousedown', document, function (e, src) {
        handleGuideCreate(e, src, false);
    });

    // 터치 이벤트
    evt.attach('touchstart', document, function (e, src) {
        handleGuideCreate(e, src, true);
    });

    function handleGuideCreate(e, src, isTouch) {
        var x, y;
        if (isTouch) {
        ...
        if (src.classList && !src.classList.contains('menu-btn')) {
            menu.close();
        }
        ...
        if (
            ...
            // 터치일 때 dragdrop.start에 터치 이벤트 전달
            dragdrop.start(isTouch ? e.touches[0] : e, guide);
