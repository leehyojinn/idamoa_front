...
import { getAdminActivePromotions, type AdminGalleryPromotion } from '@/lib/api/gallery-promotion'
import { showErrorToast } from '@/lib/errorHandler'

export default function AdminGalleryPromotionsPage() {
  const [promotions, setPromotions] = useState<AdminGalleryPromotion[]>([])
  const [isLoading, setIsLoading] = useState(true)

  const fetchPromotions = async () => {
    try {
      setIsLoading(true)
      const response = await getAdminActivePromotions()
      if (response.success && response.data) {
        setPromotions(response.data)
      }
    } catch (error) {
      showErrorToast(error, '활성 우대 목록을 불러오는데 실패했습니다')
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl min-h-[calc(100vh-64px-200px)]">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">활성 우대 갤러리</h1>
              <p className="text-gray-600 mt-1">현재 우대등록 중인 갤러리 목록</p>
            </div>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-white rounded-xl border border-gray-200">
            ...
            <p className="text-gray-500 text-lg">활성 우대 갤러리가 없습니다</p>
          </div>
        ) : (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900">갤러리</th>
                    ...
                    <th className="px-6 py-4 text-center text-sm font-semibold text-gray-900">남은일</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {promotions.map((promo) => {
                    ...
                    return (
                      <tr key={promo.promotionUuid} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className={`font-bold ${
                            promo.remainingDays <= 3
                              ? 'text-red-600'
                              : promo.remainingDays <= 7
                                ? 'text-yellow-600'
                                : 'text-green-600'
                          }`}>
                            {promo.remainingDays}일
                          </span>
                        </td>
                      </tr>
                    )
