...
import { getFeaturedGalleries, type FeaturedGallery } from '@/lib/api/gallery-promotion'

interface Props {
  count?: number
}

export default function FeaturedGalleries({ count = 8 }: Props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true)
        const response = await getFeaturedGalleries({ count })
  ...
  // 자동 슬라이드
  useEffect(() => {
    if (galleries.length <= itemsPerView) return
  ...
  }, [galleries.length, itemsPerView, maxIndex])
              ...
              >
                ...
                >
                  <div className="relative aspect-[4/3] rounded-xl overflow-hidden shadow-lg">
                    {/* 이미지 */}
                    <Image
                      src={gallery.images[0]?.fileUrl || gallery.images[0]?.thumbnailUrl || '/images/img-placeholder.png'}
                      alt={gallery.title}
                      fill
                    ...
                    {gallery.promotion && (
                      <span className={`absolute top-3 left-3 px-3 py-1 rounded-full text-xs font-bold shadow-lg ${
                        gallery.promotion.promotionType === 'PREMIUM'
                          ? 'bg-gradient-to-r from-yellow-400 to-amber-500 text-white'
                          : 'bg-gradient-to-r from-primary-500 to-primary-1000 text-white'
                      }`}>
                        {gallery.promotion.promotionType === 'PREMIUM' ? '🔥 PREMIUM' : '✨ 추천'}
                      </span>
                    )}
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform bg-[rgba(0,0,0,0.8)] z-20">
                      ...
                      {gallery.company && (
                        <div className="flex items-center gap-2 mt-1 text-white/90 text-sm">
                          <span>{gallery.company.companyName}</span>
                          {gallery.company.averageRating && (
